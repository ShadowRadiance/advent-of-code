 ...
 *
 * Your job is to find all the invalid IDs that appear in the given ranges.
...
export function part_1(input: string): string {
  ...
  ranges.forEach((range) => {
    for (let i = range.lo; i <= range.hi; i++) {
      if (!valid_part_1(i)) sum += i;
    }
  });

  return `${sum}`;
  // AOC 2025 Day 2 Part 1: 34826702005
  // deno -R aoc2025.ts 2 1  0.18s user 0.04s system 100% cpu 0.216 total
}
...
export function part_2(input: string): string {
  ...
  ranges.forEach((range) => {
    for (let i = range.lo; i <= range.hi; i++) {
      if (!valid_part_2(i)) sum += i;
    }
  });

  return `${sum}`;
  // AOC 2025 Day 2 Part 2: 43287141963
  // deno -R aoc2025.ts 2 2  0.86s user 0.04s system 101% cpu 0.886 total
}
...
function valid_part_1(value: number) {
...
function valid_part_2(value: number) {
  /*
   * Now, an ID is invalid if it is made only of some sequence of digits
   * repeated *at least* twice. So, 12341234 (1234 two times), 123123123
   * (123 three times), 1212121212 (12 five times), and 1111111 (1 seven times)
  ...
  for (let i = 1; i <= sValue.length / 2; i++) {
    const groupsOfSizeI: string[] | null = group(sValue, i);
    if (groupsOfSizeI === null) continue;

    const firstValue: string = groupsOfSizeI[0];
    if (groupsOfSizeI.every((value) => (value === firstValue))) {
      return false;
    }
  }
...
function group(s: string, size: number): string[] | null {
  // can we split evenly?
  if (s.length % size !== 0) return null;

  const array: string[] = [];

  while (s.length > 0) {
    array.push(s.slice(0, size));
    s = s.slice(size);
  }
