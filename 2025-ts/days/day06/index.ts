...
function solve(problem: string[]): number {
  const operator_c = problem.pop();
  if (operator_c === undefined) {
    throw new ArgumentError(`problem should not be empty`);
  }
  const operator = operatorFn(operator_c);
  const operands = problem.map((str) => Number.parseInt(str));
  return operands.reduce(operator);
}
